...
function guardarVenta() {
    ...
    const stockIndex = listaStock.findIndex(stock => stock.id === id && stock.talle === talle && stock.marca === marca);

    if (stockIndex === -1) {
        alert('No hay stock disponible para esta prenda');
        return;
    }

    if (listaStock[stockIndex].cantidad < cantidad) {
